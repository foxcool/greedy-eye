...
func (tc *TagCreate) AddAssetIDs(ids ...int) *TagCreate {
...
func (tc *TagCreate) AddPortfolioIDs(ids ...int) *TagCreate {
	tc.mutation.AddPortfolioIDs(ids...)
	return tc
}

// AddPortfolios adds the "portfolios" edges to the Portfolio entity.
func (tc *TagCreate) AddPortfolios(p ...*Portfolio) *TagCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.PortfoliosIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
