...
func (sq *SettingQuery) QueryUser() *UserQuery {
...
func (sq *SettingQuery) WithUser(opts ...func(*UserQuery)) *SettingQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *SettingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Setting, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, setting.ForeignKeys...)
	}
	...
	if query := sq.withUser; query != nil {
		if err := sq.loadUser(ctx, query, nodes, nil,
			func(n *Setting, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SettingQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Setting, init func(*Setting), assign func(*Setting, *User)) error {
