...
func (aq *AccountQuery) QueryHoldings() *HoldingQuery {
	query := (&HoldingClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) WithHoldings(opts ...func(*HoldingQuery)) *AccountQuery {
	query := (&HoldingClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withHoldings; query != nil {
		if err := aq.loadHoldings(ctx, query, nodes,
			func(n *Account) { n.Edges.Holdings = []*Holding{} },
			func(n *Account, e *Holding) { n.Edges.Holdings = append(n.Edges.Holdings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadHoldings(ctx context.Context, query *HoldingQuery, nodes []*Account, init func(*Account), assign func(*Account, *Holding)) error {
