...
func (s *Service) EnrichAssetData(ctx context.Context, req *services.EnrichAssetDataRequest) (*models.Asset, error) {
	...
	// Enrich from CoinGecko if it's a cryptocurrency
	if asset.Type == models.AssetType_ASSET_TYPE_CRYPTOCURRENCY {
		enrichedAsset, err := s.enrichFromCoinGecko(ctx, asset)
		if err != nil {
			s.log.Warn("Failed to enrich from CoinGecko", zap.String("asset_id", req.AssetId), zap.Error(err))
			// Return original asset if enrichment fails
			return asset, nil
		}

		// Update asset in storage with enriched data
		updateReq := &services.UpdateAssetRequest{Asset: enrichedAsset}
		updatedAsset, err := s.storageClient.UpdateAsset(ctx, updateReq)
		if err != nil {
			s.log.Error("Failed to update enriched asset", zap.String("asset_id", req.AssetId), zap.Error(err))
			return asset, nil // Return original asset if update fails
		}

		s.log.Info("Asset data enriched successfully", zap.String("asset_id", req.AssetId))
...
// enrichFromCoinGecko enriches cryptocurrency asset data from CoinGecko
func (s *Service) enrichFromCoinGecko(ctx context.Context, asset *models.Asset) (*models.Asset, error) {
	// Use CoinGecko service to get updated data
	prices, err := coingecko.Get()
	if err != nil {
	...
	for _, priceData := range prices {
		if asset.Symbol != nil && strings.EqualFold(string(priceData.BaseAsset), *asset.Symbol) {
			// Enrich asset with additional metadata
			enrichedAsset := &models.Asset{
				Id:          asset.Id,
				Symbol:      asset.Symbol,
				Name:        asset.Name,
				Type:        asset.Type,
			...
			// Add metadata as tags (since Asset model doesn't have metadata field)
			enrichedAsset.Tags = append(asset.Tags,
				"coingecko_id:"+priceData.Source,
				"data_source:coingecko",
				"enrichment_timestamp:"+priceData.Time.Format("2006-01-02T15:04:05Z"),
				"last_price_update:"+priceData.Time.Format("2006-01-02T15:04:05Z"),
			)

			return enrichedAsset, nil
		}
	}

	symbolStr := "unknown"
	if asset.Symbol != nil {
		symbolStr = *asset.Symbol
	}
	return nil, fmt.Errorf("asset %s not found in CoinGecko data", symbolStr)
}
