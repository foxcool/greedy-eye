...
var brokerUniverse = map[string]string{
	pathShares: `{"instruments":[
		{"figi":"BBG004730N88","ticker":"GAZP","name":"Gazprom","currency":"rub","lot":10,
		 "realExchange":"REAL_EXCHANGE_MOEX","apiTradeAvailableFlag":true},
		{"figi":"BBG000BPH459","ticker":"US5543821012","name":"Lowe's","currency":"usd","lot":1,
		 "realExchange":"REAL_EXCHANGE_RTS","apiTradeAvailableFlag":true},
		{"figi":"BBG00NOLOTXX","ticker":"NOLOT","name":"Lotless","currency":"rub",
		 "realExchange":"REAL_EXCHANGE_MOEX","apiTradeAvailableFlag":true},
		{"figi":"BBG00NOVENUE","ticker":"WEIRD","name":"Nowhere","currency":"rub","lot":1,
		 "realExchange":"REAL_EXCHANGE_UNSPECIFIED","apiTradeAvailableFlag":true}
	]}`,
	pathEtfs: `{"instruments":[
		{"figi":"BBG000FINEX","ticker":"IE00BK224L29","name":"FinEx","currency":"rub","lot":1,
		 "realExchange":"REAL_EXCHANGE_MOEX","apiTradeAvailableFlag":true}
	]}`,
	pathBonds: `{"instruments":[
		{"figi":"BBG00BONDXX","ticker":"SU26238","name":"OFZ 26238","currency":"rub","lot":1,
		 "realExchange":"REAL_EXCHANGE_MOEX","apiTradeAvailableFlag":true}
	]}`,
}
...
func TestCashTakesItsCurrencyFromTheTicker(t *testing.T) {
	...
	for symbol, row := range got {
		...
		// The row IS the currency, so it is also what the amount is denominated
		// in. Leaving the price's "rub" here would put the very value the
		// comment above warns against into the one field a consumer reads to
		// value the amount.
		assert.Equal(t, strings.ToLower(symbol), row.Currency,
			"%s must not be denominated in what it is priced in", symbol)
	}
	assert.Equal(t, entity.BrokerSkips{}, skips)
}

// TestBlockedQuantitySplitsThePosition: the pools must not overlap and must sum
// to what the broker reported.
//
// blockedLots is in LOTS while quantity is in PIECES — the capture proves it
// (DASB 10000 pieces to 1 lot). Subtracting one from the other reported 9999 of
// a fully blocked 10000-share holding as sellable, which is the dangerous
// direction: it tells someone money is reachable when it is not.
func TestBlockedQuantitySplitsThePosition(t *testing.T) {
	t.Run("blocked lots convert through the instrument lot size", func(t *testing.T) {
		// GAZP holds 10 pieces per lot. Three blocked lots is thirty pieces.
		rows, _ := positions(t, `{"accountId":"acc-1","positions":[
			{"figi":"BBG004730N88","instrumentType":"share","ticker":"GAZP","blocked":true,
			 "blockedLots":{"units":"3","nano":0},
			 "quantity":{"units":"90","nano":0},"quantityLots":{"units":"9","nano":0},
			 "currentPrice":{"currency":"rub","units":"140","nano":0}}
		]}`)
		...
		assert.Equal(t, "30000000000", got[entity.LiquidityLocked],
			"three lots of ten is thirty pieces, not three")
	})

	t.Run("an unknown lot size locks the whole line", func(t *testing.T) {
		// Erring towards locked understates what can be sold. Erring the other
		// way is the answer runway would be built on.
		rows, _ := positions(t, `{"accountId":"acc-1","positions":[
			{"figi":"BBG00NOLOTXX","instrumentType":"share","ticker":"NOLOT","blocked":true,
			 "blockedLots":{"units":"1","nano":0},
			 "quantity":{"units":"500","nano":0},
			 "currentPrice":{"currency":"rub","units":"2","nano":0}}
		]}`)

		require.Len(t, rows, 1)
		assert.Equal(t, entity.LiquidityLocked, rows[0].Liquidity)
		assert.Equal(t, "500000000000", rows[0].Amount)
	})
...
// TestAShortPositionIsRefusedNotDropped: every holding in this model is long,
// so a negative quantity has no shape to be written in. Letting it fall out
// silently removes a liability from the books while the response still calls
// itself complete.
func TestAShortPositionIsRefusedNotDropped(t *testing.T) {
	rows, skips := positions(t, `{"accountId":"acc-1","positions":[
		{"figi":"BBG004730N88","instrumentType":"share","ticker":"GAZP",
		 "quantity":{"units":"-5","nano":0},"currentPrice":{"currency":"rub","units":"140","nano":0}}
	]}`)

	assert.Empty(t, rows)
	assert.Equal(t, 1, skips.Unparsable, "a short is counted, not forgotten")
	assert.Equal(t, 1, skips.Total())
}

// TestARefusedPositionIsCountedOnce: a row that cannot be written must not be
// counted under two reasons at once. Recording a defaulted market for a
// position that is then refused reports a holding nobody holds.
func TestARefusedPositionIsCountedOnce(t *testing.T) {
	rows, skips := positions(t, `{"accountId":"acc-1","positions":[
		{"figi":"BBG00NOTICKER","instrumentType":"share","ticker":"","classCode":"TQBR",
		 "quantity":{"units":"10","nano":0},"currentPrice":{"currency":"rub","units":"5","nano":0}}
	]}`)

	assert.Empty(t, rows)
	assert.Equal(t, 0, skips.DefaultedMarket, "nothing was returned, so nothing was defaulted")
	assert.Equal(t, 1, skips.Unparsable)
}

// TestMetalsAreNotInventedAsCurrencies: this broker files precious metals under
// the currency instrument type. Three letters off a ticker would mint GLD and
// SLV — assets no FX source quotes, on the one path that stores no ref, so
// nothing could re-bind them later.
func TestMetalsAreNotInventedAsCurrencies(t *testing.T) {
	rows, skips := positions(t, `{"accountId":"acc-1","positions":[
		{"figi":"BBG000VJ5YR4","instrumentType":"currency","ticker":"GLDRUB_TOM","classCode":"CETS",
		 "quantity":{"units":"3","nano":0},"currentPrice":{"currency":"rub","units":"8000","nano":0}},
		{"figi":"RUB000UTSTOM","instrumentType":"currency","ticker":"RUB000UTSTOM","classCode":"CETS",
		 "quantity":{"units":"100","nano":0},"currentPrice":{"currency":"rub","units":"1","nano":0}}
	]}`)

	require.Len(t, rows, 1, "the roubles come through, the gold does not")
	assert.Equal(t, "RUB", rows[0].Symbol)
	assert.Equal(t, 1, skips.Unparsable, "gold is counted, not silently booked as a currency")
}
