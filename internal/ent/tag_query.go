...
func (tq *TagQuery) QueryPortfolios() *PortfolioQuery {
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withPortfolios: tq.withPortfolios.Clone(),
...
func (tq *TagQuery) WithPortfolios(opts ...func(*PortfolioQuery)) *TagQuery {
	query := (&PortfolioClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withPortfolios = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withPortfolios; query != nil {
		if err := tq.loadPortfolios(ctx, query, nodes,
			func(n *Tag) { n.Edges.Portfolios = []*Portfolio{} },
			func(n *Tag, e *Portfolio) { n.Edges.Portfolios = append(n.Edges.Portfolios, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Asset)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(asset.FieldID), joinT.C(tag.AssetsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.AssetsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.AssetsPrimaryKey[0]))
...
func (tq *TagQuery) loadPortfolios(ctx context.Context, query *PortfolioQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Portfolio)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Tag)
	nids := make(map[int]map[*Tag]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.PortfoliosTable)
		s.Join(joinT).On(s.C(portfolio.FieldID), joinT.C(tag.PortfoliosPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.PortfoliosPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.PortfoliosPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
